...
class BaseDASCable(Object):
    ...
    channel_spacing = Float.T(default=1.0, help="Channel spacing. Unit: [m]")
    gauge_len = Float.T(default=10.0, help="Gauge length. Unit: [m]")
    ...
    def _get_grid_offsets(self, new_grid_spacing):
        ...
        """
        if math.isclose(
            math.remainder(self.channel_spacing, new_grid_spacing),
            0.0,
            rel_tol=0.0,
            abs_tol=1e-9,
        ):
...
class SurfaceDASCable(BaseDASCable):
    ...
    def get_event_relative_data(self, event, level="channel"):
        ...
        if level == "channel":
            ...
            rlats, rlons = zip(*[c.effective_latlon for c in self.channel_locs])
        else:
...
class BoreholeDASCable(BaseDASCable):
    ...
    def set_grid_spacing(self, new_grid_spacing):
        ...
        grid_lats = np.full_like(grid_offsets, self.effective_refloc.effective_lat)
        grid_lons = np.full_like(grid_offsets, self.effective_refloc.effective_lon)
...
__all__ = ["BoreholeDASCable", "SurfaceDASCable"]
