...
def disp_from_mt(
        mt_symmat, cosine_vecs, near=True, intermed=True, far=True):
    """
    ...
    ds : :py:class:`xarray.Dataset` object
        ...
        source. The Dataset keys are `{'N', 'IP', 'IS', 'FP', 'FS'}`.
    ...
    data_vars = {
        ...
        for k, v in zip(
            ['N', 'IP', 'IS', 'FP', 'FS'],
            [A_n, A_ip, A_is, A_fp, A_fs])}

    return xr.Dataset(data_vars=data_vars, coords=coords)


def disp_from_sf(force_vec, cosine_vecs, near=True, far=True):
    """
    Parameters
    ----------
    ...
    ds : :py:class:`xarray.Dataset` object
        ...
        source. The Dataset keys are `{'N', 'FP', 'FS'}`. Each key is
    ...
    data_vars = {
        k: (dims, v.squeeze(axis=2))
        for k, v in zip(['N', 'FP', 'FS'], [A_n, A_fp, A_fs])}

    return xr.Dataset(data_vars=data_vars, coords=coords)


def normal_strain_from_mt(
        mt_symmat, cosine_vecs, near=True, near_intermed=True,
    ...
    ds : :py:class:`xarray.Dataset` object
        ...
        `{'N', 'NIP', 'NIS', 'IFP', 'IFS', 'FP', 'FS'}`. Each key is
        mapped to a 2-D array, whose shape is `(n_receivers, 3)` and
    ...
    # Near-to-intermediate field strain (NIP and NIS ∝ 1/r³)
    if near_intermed is True:
        ...
        B_nip = B_nis = np.zeros_like(Γ)

    # Intermediate-to-far field strain (IFP and IFS ∝ 1/r²)
    ...
    data_vars = {
        k: (dims, v.squeeze(axis=2))
        for k, v in zip(
            ['N', 'NIP', 'NIS', 'IFP', 'IFS', 'FP', 'FS'],
            [B_n, B_nip, B_nis, B_ifp, B_ifs, B_fp, B_fs])}
